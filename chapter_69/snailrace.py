...
MAX_NUM_SNAILS = 8
MAX_NAME_LENGTH = 20
FINISH_LINE = 10  # (!) Try modifying this number.
...
while True:  # Keep asking until the player enters a number.
    print(f'How many snails will race? Min: {MIN_NUM_SNAILS}  Max:', MAX_NUM_SNAILS)
    response = input('> ')
    if response.isdecimal():
        numSnailsRacing = int(response)
        if 1 < numSnailsRacing <= MAX_NUM_SNAILS:
            break
    print(f'Enter a number between {MIN_NUM_SNAILS} and', MAX_NUM_SNAILS)
